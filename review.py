...
if df_master.empty:
    ...
    with col_sys1:
        if st.button("🚀 サティバ (Sativa)", use_container_width=True): selected_system = "サティバ"
    with col_sys2:
        if st.button("💤 インディカ (Indica)", use_container_width=True): selected_system = "インディカ"
    ...
    if selected_system:
        new_log_row = {
            ...
            "体chainした効果": f"系統: {selected_system}", # 念のための予備キー
            "体感メモ": ""
        }
        # カラムのズレを防ぐため明示的に調整して保存
        new_log_row["体感した効果"] = f"系統: {selected_system}"
    ...
    if st.session_state.selected_effects_list:
        st.markdown(f"**【現在選択中】:** `{'` / `'.join(st.session_state.selected_effects_list)}`")
        
        c_save_eff, c_clear_eff = st.columns(2)
        with c_save_eff:
            if st.button("💾 この体感リストを確定して保存", type="primary", use_container_width=True):
                ...
                if 'save_data_to_db' in globals():
                    ...
                    st.rerun()
        with c_clear_eff:
            if st.button("❌ 選択をすべてクリア", use_container_width=True):
                st.session_state.selected_effects_list = []
                st.rerun()
    ...
    # =========================================================
    st.subheader("③ 満足度レビュー（1〜5のタップで即保存）")
    star_col1, star_col2, star_col3, star_col4, star_col5 = st.columns(5)
    
    selected_star = None
    with star_col1:
        if st.button("⭐", key="st1", use_container_width=True): selected_star = "★☆☆☆☆ (1)"
    with star_col2:
        if st.button("⭐⭐", key="st2", use_container_width=True): selected_star = "★★☆☆☆ (2)"
    with star_col3:
        if st.button("⭐⭐⭐", key="st3", use_container_width=True): selected_star = "★★★☆☆ (3)"
    with star_col4:
        if st.button("⭐⭐⭐⭐", key="st4", use_container_width=True): selected_star = "★★★★☆ (4)"
    with star_col5:
        if st.button("⭐⭐⭐⭐⭐", key="st5", use_container_width=True): selected_star = "★★★★★ (5)"
