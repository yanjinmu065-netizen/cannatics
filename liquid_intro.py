...
# 📊 アップロードされたExcel（CSV）の成分データを完全網羅したマスター辞書
INGREDIENTS_DB = {
    # --- カンナビノイド ---
    ...
    "10-OH-HHCP": {"効果": "超強力な効果、ヘッド・ボディハイ", "時間": "12〜24h", "ロケーション": "【指定薬物】完全違法", "タイプ": "半合成"},
...
if not all_liquids:
    ...
    # =========================================================
    # 🧪 【新機能】配合成分の詳細な一覧表（効果・時間・ロケーション・香り）
    # =========================================================
    st.subheader("🧪 配合成分の一覧・詳細情報")
    ...
    if detail_str:
        st.info(f"📋 **配合内容:** {detail_str}")
        
        # 配合詳細の文字列から成分が含まれているか自動マッチング
        matched_components = []
        for ing_name, ing_info in INGREDIENTS_DB.items():
            if ing_name.lower() in detail_str.lower():
                # テルペンなら「香り」、それ以外なら「ロケーション(規制状況)」を割り当てる
                loc_or_flavor = ing_info.get("香り", "ーー") if ing_info["type"] == "テルペン" else ing_info.get("ロケーション", "ーー")
                
                row_data = {
                    "区分": "🌿 テルペン" if ing_info["type"] == "テルペン" else "🧪 成分",
                    "成分名": ing_name,
                    "効果・効能": ing_info["効果"],
                    "効果時間": ing_info["時間"],
                    "ロケーション / 香り": loc_or_flavor
                }
